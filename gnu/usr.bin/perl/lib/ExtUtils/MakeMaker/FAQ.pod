...
In CVS, RCS and SVN you use $Revision: 1.1.1.4 $ (see the documentation of your
version control system for details) writing it like so:

    $VERSION = sprintf "%d.%03d", q$Revision: 1.1.1.4 $ =~ /(\d+)/g;

Every time the file is checked in the $Revision: 1.1.1.4 $ will be updated,
...
If branches are involved (ie. $Revision: 1.1.1.4 $) its a little more
complicated.

    # must be all on one line or MakeMaker will get confused.
    $VERSION = do { my @r = (q$Revision: 1.1.1.4 $ =~ /\d+/g); sprintf "%d."."%03d" x $#r, @r };
